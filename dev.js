...
const taskEnvironment = (path = require('path')) => {
  ...
  Object.assign(env, {
    ...
    PATH: [path.join(app.root, 'node_modules', '.bin'), process.env.PATH].join(path.delimiter)
  });
  return env;
};

function run(tasks) {
  Object.keys(tasks).map(name => tasks[name](name));
}

function task(
  command,
  { spawn = require('child_process').spawn, path = require('path'), color } = {}
) {
  return name => {
...
function log({ name, ts = timestamp, color = none, text = none }, out = process.stdout) {
  return data =>
    data
      .toString()
