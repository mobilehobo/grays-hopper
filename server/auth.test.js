  ...
  describe('GET /whoami', () => {
    describe('when not logged in', () =>
      it('responds with an empty object', () =>
        request(app).get('/api/auth/whoami').expect(200).then(res => expect(res.body).to.eql({}))));

    describe('when logged in', () => {
      // supertest agents persist cookies
      const agent = request.agent(app);

      beforeEach('log in', () => agent.post('/api/auth/login/local').send(alice));

      it('responds with the currently logged in user', () =>
        agent.get('/api/auth/whoami').set('Accept', 'application/json').expect(200).then(res =>
    ...
    describe('when logged in', () => {
      ...
      it('logs you out and redirects to whoami', () =>
        agent
          ...
          .then(() =>
            agent.get('/api/auth/whoami').expect(200).then(rsp => expect(rsp.body).eql({}))
          ));
    }));
});
